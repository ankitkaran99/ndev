...
def _pick_directory_os(initial_path: Optional[str] = None) -> Optional[str]:
    """Open native OS folder dialog via tkinter, zenity, or kdialog."""
    ...
    try:
        ...
        if shutil.which("zenity"):
            res = subprocess.run(
                ["zenity", "--file-selection", "--directory", f"--filename={start_dir}/", "--title=Select Document Root Directory"],
                capture_output=True,
                text=True
            )
    ...
    try:
        ...
        if shutil.which("kdialog"):
            res = subprocess.run(
                ["kdialog", "--getexistingdirectory", str(start_dir), "--title", "Select Document Root Directory"],
                capture_output=True,
                text=True
            )
            if res.returncode == 0 and res.stdout.strip():
        ...
        def _get() -> List[Dict[str, Any]]:
            ...
            try:
                ...
                for mod in get_module_manager().list_modules():
                    ...
                    pid_val = str(st.get("pid") or "-") if run else "-"
                    det = st.get("details", "")
                    web = st.get("web_ui")
                    results.append({
                        "key": f"mod:{mod.name}",
    ...
    async def action_start_all(self) -> None:
        ...
        def _do() -> None:
            ...
            try:
                ...
                self.log_message(f"[yellow]Modules notice: {e}[/yellow]")
