...
def stop_cmd(target: str = typer.Argument(None, help="PHP version or service (e.g. 8.4, pma) to stop")):
    """Stop PHP-FPM for a version or a service like phpmyadmin (pma)."""
    if target and target.lower() in ["pma", "phpmyadmin"]:
        from ndev.runtime.pma import stop_pma
        stop_pma()
        return
